...
const Education = require('./src/models/education.model');

dotenv.config({ path: path.join(__dirname, '.env') });

const seedData = async () => {
  try {
    await mongoose.connect(process.env.MONGODB_URI || 'mongodb://127.0.0.1:27017/community_help_hub');
    ...
    console.log('Created Default Admin: admin@communityhub.ap.gov.in / AdminPassword123');

    // 2. Seed Emergency Services
    const emergencies = [
      {
    ...
    console.log('Seeded state-wide Emergency numbers.');
    ...
    const hospitals = [
      {
        ...
        location: {
          type: 'Point',
          coordinates: [83.0039, 17.6896] // [longitude, latitude]
        },
    ...
    console.log('Seeded healthcare facilities & coordinates.');

    // 4. Seed Welfare Schemes
    const schemes = [
      {
    ...
    console.log('Seeded state-wide Government Welfare Schemes.');

    // 5. Seed Education Resources
    ...
    await Education.insertMany(educationResources);
    console.log('Seeded Educational Resources.');
